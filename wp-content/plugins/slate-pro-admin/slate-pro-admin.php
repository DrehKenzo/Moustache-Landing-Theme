<?php
/*
Plugin Name: Painel Customizado
Plugin URI: #
Description: Desenvolvido exclusivamente para o teste
...
if ( ! defined( 'SLATE_PRO_VERSION' ) ) {
	define( 'SLATE_PRO_VERSION', '1.1.7' );
}
if ( ! defined( 'SLATE_PRO_DB' ) ) {
	define( 'SLATE_PRO_DB', '9' );
}
...
if ( is_admin() && isset( $GLOBALS['_GET']['page'] ) && 'slate_pro_import_export' == $GLOBALS['_GET']['page'] ) {
	...
	if ( isset( $_POST['slate_pro_import'] ) ) {
		...
		$import = esc_sql( @unserialize( base64_decode( $_POST['slate_pro_import_settings'] ) ) );
...
// Uninstall Plugin
register_uninstall_hook( __FILE__, 'slate_pro_uninstall' );
function slate_pro_uninstall() {
